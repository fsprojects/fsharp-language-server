module private FSharpLanguageServer.ToolTips.XmlDoc

open System
open System.Text.RegularExpressions
open Formatting
open FSharpLanguageServer.ToolTips.Formatting
open System.Xml
...
let private handleMicrosoftOrList (text: string) =
    let splitResult =
        text.Split([| "-or-" |], StringSplitOptions.RemoveEmptyEntries)
    ...
    if Seq.length splitResult = 1 then
        ...
        |> Seq.map (fun orText ->
            ...
            // 3. We return the start of the section and the end of the section marked using quotation
                ...
                let startText =
                    orText.Substring(0, lastParagraphStartIndex)

                let endText =
                    orText.Substring(lastParagraphStartIndex)
    ...
    { TagName = "table"
      Formatter =
        ...
        | NonVoidElement (innerText, _) ->
            ...
            let rowCount =
                Regex.Matches(innerText, "<th\s?>").Count
...
type XmlDocMember(doc: XmlDocument, indentationSize: int, columnOffset: int) =
    ...
    let tabsOffset =
        String.replicate (columnOffset + indentationSize) " "
    ...
    let readChildren name (doc: XmlDocument) =
        ...
        |> Seq.cast<XmlNode>
        |> Seq.map (fun node -> Formatting.extractMemberText node.Attributes.[0].InnerText, node)
        |> Seq.toList
    ...
    let parameters =
        rawParameters |> List.map readNamedContentAsKvPair

    let remarks =
        rawRemarks |> Seq.map readContentForTooltip

    let exceptions =
        rawExceptions |> List.map readNamedContentAsKvPair

    let typeParams =
        rawTypeParams |> List.map readNamedContentAsKvPair

    let examples =
        rawExamples |> Seq.map readContentForTooltip
    ...
    let seeAlso =
        ...
        |> Seq.map (fun node ->
            "* `"
            + Formatting.extractMemberText node.Attributes.[0].InnerText
            + "`")
    ...
    if xmlDocCache.ContainsKey xmlFile then
        ...
        let rec exists filePath tryAgain =
            ...
            | false, true ->
                ...
                let filePath =
                    Path.ChangeExtension(filePath, Path.GetExtension(filePath).ToUpper())
